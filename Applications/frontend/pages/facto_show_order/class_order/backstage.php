...
$categorize = new tree(
    [
        ...
        ['child_key' => ['dish' ,'department' ,'id']         ,'func' => "department_dom"],
...
function department_dom($parent_id ,$content_id ,$content ,$value){
    ...
    $title_dom = '<div class="father"><div class="title"><h4 class=" panel-title"><a style="width:80%" data-toggle="collapse" data-parent="#' . $parent_id . '" href="#' .
        $content_id . '">' . $title . '</a></h4></div><div class="checker ' . $code . '" code="' . $code  . '"><label class="switch"><input type="checkbox" ' .
        ($value->info->paid['factory'] ? " checked " : " " ) . ' ><span class="slider"></span></label></div></div>';
    return '<div class="panel panel-default"><div class="panel-heading">' . $title_dom . 
        '</div><div id="' . $content_id . '" class="panel-collapse collapse"><div class="panel-body">' . $content .
        '</div></div></div>';
}
