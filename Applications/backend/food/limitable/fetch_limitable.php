...
{
    $mysqli = $_SESSION['sql_server'];
    $sql = "SELECT D.id ,IF(F.daily_limit = -1 ,D.daily_limit , IF(F.daily_limit > F.sum ,IF(D.daily_limit = -1 ,-1 ,D.daily_limit-D.sum) ,0)) ,IF(F.daily_limit = F.sum ,D.daily_limit ,D.sum) ,D.last_update
