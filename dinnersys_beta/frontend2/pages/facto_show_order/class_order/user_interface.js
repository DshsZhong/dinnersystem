...
var fid = window.localStorage.fid;
var adapt = {
    "1" : "台灣小吃部" ,
    "2" : "早餐部" ,
    "3" : "愛佳便當" ,
...
function make_element(categorized) {
    ...
    for(var k1 in sorter) {
        ...
        for(var k2 in class_value['data']) {
            ...
            for(var k3 in time_value['data'][fname]['data']) {
                var dish_value = time_value['data'][fname]['data'][k3];
                var content_3 = "";
        ...
        content.push({
            "title" : inner_item['user']['class_no'] + " x" + class_value['other']['order_sum'] + "(" + class_value['other']['charge_sum'] + "$.)" ,
...
function load() {
    ...
    var url = "../../../../backend/backend.php?cmd=select_facto&dm=true&cafet=true&facto=false&esti_start=" + today + "-00:00:00&esti_end=" + today + "-23:59:59&factory_id=" + fid;
